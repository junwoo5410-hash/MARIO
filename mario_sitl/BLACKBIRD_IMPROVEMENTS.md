...
**정정(2026-09-08)**: 이 표의 제목은 원래 "AirIO와 동일 구간"이었으나 실측 결과
사실이 아니다. 보폭 999는 체인 끊김을 고치지만 **꼬리는 여전히 버린다** — 윈도우는
1000 샘플이 온전히 남아야 시작하므로 남은 구간이 그보다 짧으면 평가되지 않는다.
궤적별 커버리지는 48.6 %(winter) ~ 99.4 %(clover)로 고르지 않다. unseen에서는 그 위에
로더가 만든 시퀀스 길이 자체가 AirIO와 다르다(sid 49.9 vs 38.2 m, oval 65.0 vs 48.6 m).
따라서 아래 unseen 비교를 "동일 구간"으로 서술하면 안 된다. 실측표는 `PAPER_NOTES.md`
§4.3에 있다.
...
데이터를 한 번도 보지 않는 파라미터 19,392개(인코더 15,296 +
융합 Linear의 64×64=4,096)를 들고 있었다.
